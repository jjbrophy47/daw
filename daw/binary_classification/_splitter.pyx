...
cdef SIZE_t select_greedy_threshold(Node*     node,
    ...
    cdef Feature*   second_best_feature = NULL
    cdef Threshold* second_best_threshold = NULL
    ...
    while n_features < max_features and (sampled_features.n + constant_features.n) < n_total_features:
        ...
        n_candidate_thresholds = get_candidate_thresholds(feature_index, values, labels, indices, samples.n,
        ...
        while sampled_indices.n < n_candidate_thresholds_to_sample:
            ...
            if valid:
                ...
                # compute split score
                split_score = compute_split_score(use_gini, node.n_samples, threshold)
...
cdef SIZE_t select_random_threshold(Node*     node,
    ...
    while sampled_features.n + constant_features.n < n_total_features:
        ...
        # non-constant feature
            ...
            node.chosen_threshold = create_threshold(feature_index, threshold_value, n_left_samples, n_right_samples)
            node.constant_features = copy_intlist(constant_features, constant_features.n)
...
cdef SIZE_t get_candidate_thresholds(SIZE_t       feature,
    ...
    for k in range(1, feature_value_count):
        ...
        if ((v1_label_ratio != v2_label_ratio) or
            ...
            # create threshold
            threshold = create_threshold(feature, v1, n_left_samples, n_right_samples)
