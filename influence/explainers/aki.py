...
class AKI(Explainer):
    ...
    """
    def __init__(self, k=1, n_jobs=-1, logger=None):
        """
        Input
    ...
    def get_local_influence(self, X, y):
        ...
        return self._run(X_test=X, y_test=y)

    # private
    def _run(self, X_test, y_test):
        """
        ...
        with joblib.Parallel(n_jobs=self.n_jobs_) as parallel:
            ...
            while fits_remaining > 0:
                ...
                results = parallel(joblib.delayed(_run_iteration)
                    ...
                    train_idx, self.k, X_test, y_test, self.objective_, self.loss_fn_,
...
def _run_iteration(model, X_train, y_train, train_idx, k, X_test, y_test,
    ...
    """
    add_X = np.tile(X_train[train_idx], (k, 1)).astype(dtype=np.float32)
    add_y = np.tile(y_train[train_idx], k).astype(dtype=np.float32)
    new_X_train = np.concatenate((X_train, add_X))
    new_y_train = np.concatenate((y_train, add_y))
